...
client.on("ready", () => {
  ...
  client.user.setGame(`p!help | By ا̍حۡــمۘــد#3933`);
});
...
client.on('message', message => {
    if (message.content.startsWith("!pinvserver")) {
...
client.on('message', message => {
  ...
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/SyFD2rU**")
...
client.on('guildMemberAdd', member=> {
    member.addRole(member.guild.roles.find("name","◦ PIXCL » MEMBERS"));
    });
...
The bot was made By ا̍حۡــمۘــد#3933
`)
